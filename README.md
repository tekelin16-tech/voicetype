...
同音字（國翔／國祥）光靠熱詞表壓不住——辨識選哪個字是機率問題。所以分兩層：

1. **熱詞表**（設定裡的「專有名詞熱詞表」）— 餵給 whisper，提高一開始就聽對的機率。
2. **修正字典**（設定裡的「修正字典」）— 事後把漏網的改掉。一行一條：
   - `國翔` — 告訴 DeepSeek 這是專有名詞，聽到音近的就往這邊修
   - `國祥→國翔` — 不管前面怎麼判，最後一定換掉（最保險）

存在 `~/.config/voicetype/corrections.txt`。替換時長的規則先套用，
免得短規則把長規則的字先吃掉。
