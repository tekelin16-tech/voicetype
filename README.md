...
DeepSeek 的 flash 層常常塞車（回 503 或直接不回應）。三層防護，你不用管：

| 情況 | 行為 |
|---|---|
| 主模型不通 | 自動改用備援，並記下時間，**4 小時內直接走備援**不再浪費時間試 |
| 4 小時後 | 自動回頭試主模型，通了就切回來並通知 |
| 兩個都不通 | **暫停呼叫 10 分鐘**，那段時間直接秒出未整理的原文 |

逾時分開設定：主模型 8 秒（有備援可退，短皮帶），備援 25 秒（最後一道）。
設得太長的話，等滿跟當掉沒兩樣——這是實際踩過才調的。

**設定視窗最上面會顯示現在的狀態**：綠燈正常、黃燈備援中、紅燈暫停中，
並列出目前用哪個模型、幾分鐘後試著切回、以及**最近一次的錯誤訊息**。

最後那行是關鍵：`HTTP 503 Service is too busy` 代表那個模型忙線（換模型有用），
`逾時（8s 內無回應）` 代表整個 API 沒反應（換模型也沒用）。要接什麼備援看這行決定。

冷卻或暫停時右上角有「立即重試」，服務恢復了不用等滿。
選單列的圖示點開也有一行即時狀態。指令列用 `vt.sh model-status`。
設定在 `config.sh`：`DS_MODEL`、`DS_FALLBACK`、`DS_COOLDOWN`、`DS_DOWN_COOLDOWN`。

## 貼上的行為

**除了密碼欄位，一律直接貼。** 密碼欄位會拒絕，文字留在剪貼簿讓你自己決定。

判斷不出焦點在哪的時候（很多 App 不回報，或回報成 `AXWindow`、`AXCell`），
還是照貼，但**不會還原你原本的剪貼簿**——文字留著當保險，萬一沒貼進去，
按 ⌘V 就救得回來。確定是輸入框時才會還原剪貼簿。

第一版用的是「認出不能打字就不貼」的黑名單，誤判太多：明明點在輸入框上卻不幫你貼，
比偶爾貼錯地方糟得多。每次的判斷都會寫進 `~/.cache/voicetype/vt.log`，
遇到怪狀況可以直接看是哪個 App、回報什麼角色。
...
- **`$變數` 後面直接接中文，bash 會把中文當成變數名的一部分**。
  `"$DS_FALLBACK，改用"` 會變成找變數 `DS_FALLBACK，`，`set -u` 下直接炸。
