...
```bash
~/.local/share/voicetype/scan-vocab.py ~/Desktop         # 列出候選
~/.local/share/voicetype/scan-vocab.py ~/Desktop --add   # 直接加進詞彙表
```

掃描你自己寫的 md／txt，挖出常用的專有名詞。

**為什麼不攔截鍵盤**：那會把密碼、私訊、金融資料全部錄下來存在硬碟。
你電腦裡本來就有一堆親手打的字，那是同樣準確而且零風險的樣本。

判斷「這是不是你寫的」用中文比例（低於 8% 的多半是第三方英文文件），
並跳過 LICENSE／CHANGELOG——第一版沒濾，結果前 30 名全是
`WARRANTIES`、`Copyright`、`INCLUDING` 這種授權樣板字。

排序偏好：被「」框起來的詞（作者刻意標記，訊號最強）、英數混合的品牌名、
三字以上的中文詞。兩個字的中文詞除非被引號框過否則跳過——那些絕大多數是
「完整」「內容」「時間」這類通用詞，辨識本來就對，佔熱詞表名額不划算。
已經在口述紀錄裡正確出現過的詞也會跳過，同樣理由。
...
| 情況 | 行為 |
|---|---|
| 主模型不通 | 自動改用備援，並記下時間，**冷卻期內直接走備援**不再浪費時間試 |
| 冷卻期滿 | 自動回頭試主模型，通了就切回來並通知 |
...
**「測試連線」用的是跟真實口述一樣大的請求**（系統指令＋詞彙表約 1500 字元）。
小請求（`"ok"` + max_tokens 8）在 flash 塞車時照樣擠得進去，測起來永遠正常，
但真正的請求會失敗——測試跟真實情況不一樣的話，測了等於沒測。
